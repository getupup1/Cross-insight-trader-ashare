...
class StockPortfolioEnv(gym.Env):
    ...
    def __init__(
        ...
        # self.observation = np.array(self.data['dec_price_list'].values[0])
        self.terminal = False
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            self.reward = mh.log(round(new_portfolio_value/self.asset_memory[-2],5))
            # self.reward = mh.log(round(new_portfolio_value/self.asset_memory[0],5)) + \
            #                 mh.log(round(new_portfolio_value/self.asset_memory[0],5)/self.asset_ratio[-1])
            # self.reward = mh.log(round(new_portfolio_value/self.asset_memory[0],5)) + \
            #               0.01 * mh.log(round(new_portfolio_value/self.asset_memory[-2],5))
